...
//	return true;
	...
	if (this->fd_tty <= 0)	{
		LOGI("Com::start - open device %s faild\n", UART_DEVICE_NAME);
		return false;
	}

	if(this->set_tty()!=0){
		LOGI("Com::start - fail to set parity\n");
		this->stop();
		return false;
	}
	...
	if(!CREATE_THREAD(&threadId,staticReceivethread,this)){
		LOGI("Com::start - create com thread error\n");
		this->stop();
		return false;
	}
	if(!CREATE_THREAD(&threadId,staticOpenDoorThread,this)){
		LOGI("Com::start - create com open door thread error\n");
		this->stop();
		return false;
	}
	return true;
...
THREAD_TYPE Com::OpenDoorThread(){
